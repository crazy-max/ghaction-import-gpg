  ...
  try {
    ...
    if (inputs.gitUserSigningkey) {
      ...
      if (!privateKey.allUserIds.some(id => id.email === userEmail)) {
        ...
        core.setFailed(`Committer email "${inputs.gitCommitterEmail}" (name: "${inputs.gitCommitterName}") does not match any of the GPG private key user id email addresses: ${keyIdentities}`);
        return;
      }
